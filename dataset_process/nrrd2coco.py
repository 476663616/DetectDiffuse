...
# -----
# Last Modified: 2024-10-02 20:55:13
...
if __name__ == '__main__':
    img_path = 'V:/lesions/train/'
    seg_path = 'V:/lesions/train/'
    png_save_path = 'V:/lesions/lnq_coco/images/'
    os.makedirs(png_save_path, exist_ok=True)
    coco_save_path = 'V:/lesions/lnq_coco/'
    image_cnt = 0
    bbox_cnt = 0
    for image in os.listdir(img_path):
        if 'ct' in image:
            ...
            seg = sitk.ReadImage(seg_path + image.replace('ct','seg'))
            img_arr = sitk.GetArrayFromImage(img)
            seg_arr = sitk.GetArrayFromImage(seg)
            # seg_arr[seg_arr<2]=0
            slices = list(set(np.where(seg_arr == 1)[0].tolist()))
